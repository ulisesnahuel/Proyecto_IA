...
b1=x(size(x))-x(1);%se calcula la base del primer triángulo
h1=y((size(y)))-y(1);%se calcul la altura del primer triángulo
if(h1(2)<0)% si la altura es negativa se cambia el signo
    h1=-h1;
end
a1=(b1(2)*h1(2))/2;%se calcula el área del primer triángulo
b2=x1(size(x))-x1(1);%se calcula la base del segundo triángulo
h2=y1((size(y1)))-y1(1);%se calcula la altura del segundo triángulo
if(h2(2)<0)%si la altura es negativa se cambia el signo
    h2=-h2;
end
a2=(b2(2)*h2(2))/2; %se calcula el área del segundo triángulo
...
b1=x(size(x))-x(1); %se calcula la base del primer triángulo de la figura redimensionada
h1=y2((size(y2)))-y2(1);%se calcula la altura del primer triángulo de la figura redimensionada
if(h1(2)<0)%se cambia de signo si la altura es negativa
    h1=-h1;
end
a1=(b1(2)*h1(2))/2;%se calcula el área del primer triángulo de la figura redimensionada
b2=x1(size(x))-x1(1);%se calcula la base del segundo triángulo de la figura redimensionada
h2=y3((size(y3)))-y3(1);%se calcula la altura del segundo triángulo de la figura redimensionada
if(h2(2)<0)
    h2=-h2;
end
a2=(b2(2)*h2(2))/2;%se calcula el área del segundo triángulo de la figura redimensionada
